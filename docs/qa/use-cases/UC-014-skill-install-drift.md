...
**Expected**
- Claude Code and Cursor install exactly **19 shared skills** to the skills
  dir — the coordinator (`thatch-code-review`) is **absent** (it needs
  sub-agents, which those hosts lack).
- opencode installs **20** — the 19 shared plus the coordinator.
...
_Automatable: yes — file presence, count (19 vs 20), content-diff-overwrite, and
