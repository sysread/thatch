...
**Expected**
- Claude Code and Cursor install exactly **20 shared skills** to the skills
  dir — the coordinator (`thatch-code-review`) is **absent** (it needs
  sub-agents, which those hosts lack).
- opencode installs **21** — the 20 shared plus the coordinator.
...
_Automatable: yes — file presence, count (20 vs 21), content-diff-overwrite, and
