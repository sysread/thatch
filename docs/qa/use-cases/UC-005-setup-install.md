...
- `--claude` project-local writes `.mcp.json` (`mcpServers.thatch`, stdio + `["mcp"]`),
  ...
  **20 shared skills** (no code-review coordinator) to `$CLAUDE_CONFIG_DIR/skills/` — user-scoped
...
- `--cursor` project-local writes `.cursor/mcp.json`, appends to `AGENTS.md`, writes
  ...
  `thatch flush-tools --json`; and installs 20 shared skills to `~/.cursor/skills/`.
