...
| `thatch_chat_list` | List registered sessions grouped into Active and Stale sections (stale = no report for over ten minutes; its harness may be gone), with how long ago each last checked in, each session's project and checkout kind (project root or linked worktree), its topic, and your unread count. |
