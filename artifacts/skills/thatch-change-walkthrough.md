...
description: Explain a change to the user as a teaching walkthrough. Resolves the change range against the default branch (merge-base..HEAD), researches each business-logic workflow the change touches at the merge-base state of the code, then teaches the current behavior and overlays how the change modifies it with file:line citations, plain-English analogies, and bold/italic save points. Use when the user asks "walk me through this change", "explain what this PR does", "I want to understand what this branch changes before reading the diff", or otherwise wants to learn the code touched by a change. Do NOT use for drafting PR descriptions, code review, or standalone workflow research.
---
