 ...
 *
 * Automatable: yes — `extraction.test.ts` (in-memory per-session) and
 * `extract-queue.test.ts` (file-backed per-session) already cover isolation;
 * the sideband is a stateless request/response so concurrency is trivially
 * safe. Requires a live opencode session for the end-to-end runbook.
 */
