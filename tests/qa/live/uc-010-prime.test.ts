...
const useCase: UseCase = {
  ...
  expected: [
    "- `prime` resolves the first available CLI in order — `opencode run`, then `agent -p --approve-mcps` (Cursor), then `claude` — and spawns it with the project-primer prompt.",
    "- The primer skill recalls existing memories first, investigates the codebase from multiple angles, and writes foundational memories via `thatch_memory_remember` (project vs global assignment per its own rules), then runs dedup afterward.",
