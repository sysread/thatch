...
const useCase: UseCase = {
  ...
  expected: [
    ...
    "  **25 shared skills** (no code-review coordinator) to `$CLAUDE_CONFIG_DIR/skills/` — user-scoped",
    ...
    "  `thatch flush-tools --json`; and installs 25 shared skills to `~/.cursor/skills/`.",
  ...
  async run(ctx: QaContext) {
    ...
    // Claude skills: 25 shared, no coordinator, under $CLAUDE_CONFIG_DIR/skills/
    ...
    if (claudeSkillNames.length !== 25) {
      console.log(`  FAIL: Claude skills count is ${claudeSkillNames.length}, expected 25`);
      return "FAIL";
    }
    ...
    // Cursor skills: 25 shared, no coordinator, under ~/.cursor/skills/
    ...
    const cursorSkillNames = readdirSync(cursorSkillsDir, { withFileTypes: true })
      ...
      .map((d) => d.name)
      .filter((n) => n.startsWith("thatch-"));
    if (cursorSkillNames.length !== 25) {
      console.log(`  FAIL: Cursor skills count is ${cursorSkillNames.length}, expected 25`);
      return "FAIL";
    }
