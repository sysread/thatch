 ...
 *
 * Automatable: yes — file presence, count (26 vs 27), content-diff-overwrite,
 * and coordinator host-gating are all file assertions (`setup.test.ts` already
 * covers the unit contract). The opencode skill count (26) is verified against
...
const useCase: UseCase = {
  ...
  async run(ctx: QaContext) {
    ...
    // opencode: 27 (26 shared + 1 coordinator), pre-populated by ensureMaster
    ...
    if (existsSync(opencodeSkillsDir)) {
      const opencodeSkills = readdirSync(opencodeSkillsDir, { withFileTypes: true })
        ...
        .map((d) => d.name)
        .filter((n) => n.startsWith("thatch-"));
      if (opencodeSkills.length !== 27) {
        console.log(`  FAIL: opencode skills count is ${opencodeSkills.length}, expected 27`);
        return "FAIL";
      }
